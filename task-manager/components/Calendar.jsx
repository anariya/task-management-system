...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isToday as isTodayDate, addMonths, subMonths, setMonth, setYear, getYear, getMonth, startOfWeek, endOfWeek, isSameMonth, subWeeks, addWeeks, subDays, addDays } from 'date-fns';
...
const Calendar = ({groupID}) => {
  ...
  const handleEventClick = (event) => {
    ...
    if (event.editable === "true") {
      ...
      console.log(new Date(events[0].start_date).getDate());
      console.log(new Date(events[0].start_date).getUTCDate());
  ...
  const handleCloseEventDetail = () => {
    ...
    setIsEditing(false);
    setNewEvent({ name: '', startDate: '', endDate: '', location: '', groupId: groupID });
    setStartDateInput('');
    setStartTimeInput('');
    setEndDateInput('');
    setEndTimeInput('');
  ...
  const handleAddEvent = async (e) => {
    ...
    const startDateTimeLocal = new Date(new Date(startDateTime.getTime()) - timezoneOffset);
    const endDateTimeLocal = new Date(new Date(endDateTime.getTime()) - timezoneOffset);
    ...
    if (startDateTimeLocal.toISOString().split("T")[0] !== endDateTimeLocal.toISOString().split("T")[0]) {
  ...
  const handleDeleteEvent = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEvents(events.filter(event => event["event_id"] !== tmpEvent.eventId));
        handleCloseModal();  // Close the modal after updating
        handleCloseEventDetail();
        await fetchEvents();
      } else {
  ...
  const getEventBlocks = () => {
    ...
    return events
      .filter(event => {
        ...
        const isSameDayEnd = endDate.getUTCFullYear() === currentDateObj.getUTCFullYear() &&
    ...
    <div>
      <header className={styles["calendar-header"]}>
                ...
                <div className={styles["dropdown-column"]}>
                  ...
                  >
                    {Array.from({ length: 12 }, (_, i) => i).map(month => (
                      <option key={month} value={month}>{format(new Date(2020, month), 'MMMM')}</option>
                    ))}
                  </select>
                </div>
              </div>
              <button className={styles["apply-button"]} onClick={applySelection}>
                Apply
              </button>
            </div>
          )}
        ...
        <div className={styles["calendar-controls"]}>
          <div className={styles["prev-next-group"]}>
            <button className={styles["prev-next-button"]} onClick={handlePrevMonth}>
              &lt;
            </button>
            <button className={styles["prev-next-button"]} onClick={goToToday}>
              Today
            </button>
            <button className={styles["prev-next-button"]} onClick={handleNextMonth}>
              &gt;
            </button>
          </div>
          <button className={styles["add-event-button"]} onClick={handleOpenModal}>
            Add Event
          </button>
        </div>
      </header>
            ...
            return (
              <div
                ...
                className={`${styles["calendar-day"]} ${
                  ...
                  isSameMonth(day, currentDate) ? styles["current-month"] : styles["other-month"]
                }`}
              >
                ...
                {dayEvents.length > 0 && (
                  <div className={styles["task-list"]}>
                    {dayEvents.map((event, index) => (
                      <div key={index} className={styles["task-item"]} onClick={() => handleEventClick(event)}>
                        {event.name}
                      </div>
                    ))}
                  </div>
      ...
      {activeView === 'Day' && (
        <div className={styles["schedule-container"]}>
          <div className={styles["date"]}>{currentDate.toLocaleString('en-US', { weekday: 'short', day: 'numeric' })}</div>
          ...
          <div className={styles["weekly-schedule"]}>
            ...
            {daysOfWeek.map((day, index) => (
              <div className={styles["day-column"]} key={index}>
                <div className={styles["day-header"]}>{day.toLocaleString('en-US', { weekday: 'short', day: 'numeric' })}</div>
                <div className={styles["all-day-event-row"]}>
                  {getAllDayEventsForDay(day)}
                </div>
                <div className={styles["day-schedule"]}>{getEventBlocksForDay(day)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {isEventOpen && selectedEvent && (
        <div className={styles["modal"]}>
          <div className={styles["modal-content"]}>
            ...
            <p><strong>Start Date:</strong> {format(new Date(selectedEvent.start_date), 'PPPP')}</p>
            <p><strong>End Date:</strong> {format(new Date(selectedEvent.end_date), 'PPPP')}</p>
            <p><strong>Location:</strong> {selectedEvent.location}</p>
            <button type="button" onClick={handleCloseEventDetail}>Close</button>
          </div>
        </div>
      )}
