...
const Calendar = ({ groupID }) => {
  ...
  const getEventsForDay = (date) => {
    ...
    return events.filter((event) => {
      ...
      return (
        formattedDate >=
          `${startDate.getUTCFullYear()}-${String(
            startDate.getUTCMonth() + 1
          ).padStart(2, "0")}-${String(startDate.getUTCDate()).padStart(
            2,
            "0"
          )}` &&
        formattedDate <=
